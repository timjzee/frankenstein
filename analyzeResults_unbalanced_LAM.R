known_dataset <- read.csv("/Users/tim/GitHub/frankenstein/results/check4/equalized_results_10-cv_BOTH.csv")
...
known_dataset$macro_precision = (known_dataset$MWS_precision + known_dataset$LAM_precision) / 2
known_dataset$MWS_recall = known_dataset$true_MWS_pred_MWS / (known_dataset$true_MWS_pred_MWS + known_dataset$true_MWS_pred_LAM)
known_dataset$LAM_recall = known_dataset$true_LAM_pred_LAM / (known_dataset$true_LAM_pred_LAM + known_dataset$true_LAM_pred_MWS)
known_dataset$macro_recall = (known_dataset$MWS_recall + known_dataset$LAM_recall) / 2
...
known_dataset$accuracy = (known_dataset$true_MWS_pred_MWS + known_dataset$true_LAM_pred_LAM) / 100
...
franken_dataset = read.csv("/Users/tim/GitHub/frankenstein/results/check4/franken_results_groups_BOTH.csv")
...
franken_aov = with(franken_dataset2, aov(percentage_LAM ~ sample_size * feature_type + Error(group / (sample_size * feature_type))))
summary(franken_aov)

interaction.plot(franken_dataset2$sample_size, franken_dataset2$feature_type, franken_dataset2$percentage_LAM, xlab = "Sample Size", ylab = "Percentage classified as LAM", trace.label = "", leg.bty = "o")
